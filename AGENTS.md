...
아키텍처 또는 도메인을 변경하기 전에 관련 문서를 확인한다.
...
설계 문서와 작업 보고에서는 애그리거트, 애그리거트 루트, 불변식, 생명주기 상태,
실행 차단 사유, 도메인 행위, 애플리케이션 서비스, 조회 모델, 소유권 인가라는 용어를
일관되게 사용한다. Java 코드 식별자는 변경하지 않는다.
