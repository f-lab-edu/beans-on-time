...
사용자 요청과 기존 확정 규칙 안에서 해결할 수 있는 구현 선택은 기존 관례에 따라
자율적으로 진행한다. 필요한 가정과 선택 이유는 작업 보고에 짧게 남긴다.

확정된 규칙과 충돌하거나 새로운 비즈니스 결정이 필요한 부분은 충돌 내용과 선택지를
제시하고 확인한다. 답변이 필요한 부분은 보류하되 독립적으로 진행할 수 있는 작업은
계속한다. 사용자가 이미 명시적으로 결정한 사항은 재확인하지 않는다. 기존 규칙의 변경을
명시적으로 결정했다면 그 결정에 따라 관련 문서와 구현을 함께 갱신한다.

검토만 요청받으면 지적과 수정 방향을 보고하고 파일을 수정하지 않는다. 기존 코드나
문서의 감사 요청에서는 사용자가 지정한 범위의 기존 문제도 검토한다.

## 유즈케이스 단위 구조

서로 다른 비즈니스 행위는 기본적으로 유즈케이스별 구조로 설계한다.

- 입력 포트와 애플리케이션 서비스는 원칙적으로 하나의 유즈케이스에 1:1로 대응시킨다.
- 인바운드 어댑터는 요청을 해당 입력 포트의 명시적인 행위에 연결한다.
- 출력 포트는 구현 기술이 아니라 각 유즈케이스가 필요로 하는 능력을 기준으로 나눈다.
- 어댑터도 포트와 유즈케이스의 책임과 변경 이유가 다르면 분리하는 것을 우선한다.
- 단, 동일한 저장소나 외부 API 메커니즘을 공유하고 책임 경계를 흐리지 않는 경우에는
  하나의 아웃바운드 어댑터가 여러 관련 출력 포트를 구현할 수 있다.

의존성이 같거나 코드 모양이 비슷하다는 이유만으로 서로 다른 유즈케이스를 하나의 서비스나
어댑터로 합치지 않는다. 반복을 줄이기 위한 공통 추상화는 실제로 같은 도메인 의미와
변경 이유가 확인될 때만 도입한다.
...
리뷰 결과는 `PASS`, `NEEDS_FIX`, `BLOCKED`로 구분한다. 지적이 없으면 리뷰를 종료한다.
지적을 수정한 뒤의 재리뷰는 기본 최대 두 차례로 제한한다. 이는 필수 반복 횟수가 아닌
상한이다. 그 뒤에도 리뷰 지적이 남거나 같은 지적이 반복되면 임의로 변경 범위를 넓히지
않고 남은 위험과 필요한 결정을 보고한다.

최종 변경 상태에서 필수 검증이 통과했다면 새 변경, 실패 또는 미해결 우려 없이 검증을
확대하거나 반복하지 않는다. 문서만 변경한 작업은 Java 테스트 대신 Markdown 형식,
내부 경로와 관련 규칙의 일관성을 확인한다.
...
문서의 한글 표현을 정리하기 위한 목적으로 Java 식별자를 변경하지 않는다. 식별자 변경은
사용자 요청 범위와 실제 코드 계약에 따라 판단한다.
