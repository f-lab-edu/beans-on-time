...
소스 코드의 의존 관계는 다음과 같다. 화살표는 의존하는 대상 또는 구현하는 인터페이스를
향한다.

~~~text
인바운드 어댑터 → 입력 포트 ← 애플리케이션 서비스
                              ├→ 도메인
                              └→ 출력 포트 ← 아웃바운드 어댑터
~~~

애플리케이션 서비스는 입력 포트를 구현하고 도메인 행위와 출력 포트 호출을 조율한다.
도메인은 애플리케이션 계층의 출력 포트에 의존하지 않는다. 런타임에는 입력 포트를 통해
서비스가 호출되고, 서비스가 출력 포트를 통해 아웃바운드 어댑터를 호출한다.
...
현재 관례에 따라 애플리케이션 서비스의 Spring 빈 등록을 위한 `@Service`를 허용한다.
이는 구성용 어노테이션에 한정한 허용이며, 포트 계약과 도메인은 순수 Java로 유지한다.
애플리케이션 서비스는 HTTP, Spring Security, 영속성 고유 API나 어댑터 구현에 직접
의존하지 않고 필요한 기능을 포트로 요청한다.

기존 서비스 구성 방식을 유지하면서 비즈니스 규칙과 외부 기술의 경계를 보존하기 위한
선택이다. 이 허용을 근거로 다른 프레임워크 의존성을 선제적으로 도입하지 않는다.
