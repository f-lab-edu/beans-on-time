...
public class ResumeSubscriptionServiceTest {
    ...
    @Mock
    OwnedSubscriptionLoader ownedSubscriptionLoader;

    @InjectMocks
    ResumeSubscriptionService resumeSubscriptionService;

    @Test
    void 재개된_구독을_저장한다() {
        // given
        ResumeSubscriptionCommand command = new ResumeSubscriptionCommand(new SubscriptionId(UUID.randomUUID()), new CustomerId(1L));
        Subscription subscription = createPausedSubscription();

        given(ownedSubscriptionLoader.loadOwnedSubscription(command.customerId(), command.subscriptionId()))
