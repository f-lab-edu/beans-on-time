...
public class SubscriptionAccessPolicyTest {
  private final SubscriptionAccessPolicy accessPolicy = new SubscriptionAccessPolicy();
  ...
  void 구독_소유자는_접근할_수_있다() {
    ...
    // when & then
    assertThatCode(() -> accessPolicy.validateOwner(subscription, ownerCustomerId))
  ...
  void 구독_소유자가_아니면_접근할_수_없다() {
    ...
    // when & then
    assertThatThrownBy(() -> accessPolicy.validateOwner(subscription, otherCustomerId))
